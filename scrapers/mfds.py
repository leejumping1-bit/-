...
MFDS 게시판은 공통적으로 전자정부 표준 게시판(eGovFrame) 구조를 따르며,
목록은 <table> 안에 행(tr)마다 제목 링크(a[href*="view.do?seq="])와 등록일 셀을 갖는다.
개별 게시글 뷰(view.do?seq=NNNN)가 원문(직접링크)이며, 이 스크립트가 접근에 실패하면
(예: GitHub Actions 해외 IP 차단) source_type 을 'fallback' 으로 낮추고
목록 페이지 URL을 대신 기록한다 — README의 Self-hosted Runner 안내 참고.
"""
import re
import sys
import os
...
def crawl_board(board_key, board_url, since_year=2026, since_month=1, max_pages=3):
    ...
    본문 상세 파싱(고시번호, 시행일 등)은 각 게시글 상세 페이지에서 추가로 조회해야
    정확하지만, 이 스캐폴드에서는 목록 단계 수집까지만 구현하고
    상세 파싱은 TODO로 남겨 실제 실행 환경에서 HTML 구조를 검증한 뒤 완성해야 한다.
    """
    ...
    for page in range(1, max_pages + 1):
        ...
        for a in soup.select('a[href*="view.do"]'):
            ...
            # 같은 행(tr) 내 날짜 텍스트 탐색
            tr = a.find_parent("tr")
            date_text = tr.get_text(" ", strip=True) if tr else ""
